...
questions = {
 ...
 questions.each do |q, details|
  ...
  exists = q && q.section.to_s == details[:section] && q.section.organisation == details[:organisation]
  if !exists
...
questions_locales = {
 ...
 questions_locales.each do |q, details|
   q = Question::Translation.find_by_text(details[:text])
   exists = q && Question.find(q.question_id).section == details[:section] && Question.find(q.question_id).section.organisation == details[:organisation]
   if !exists
